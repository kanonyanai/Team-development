...
function initAutocomplete() {
  ...
  //経路表示のUIリンク
  //from
  const input_a = document.getElementById('startword');
  const roteBox_from = new google.maps.places.SearchBox(input_a);
  map.addListener("bounds_changed", () => {
    roteBox_from.setBounds(map.getBounds());
  });

  //to
  const input_b = document.getElementById('endword');
  const roteBox_to = new google.maps.places.SearchBox(input_b);
  map.addListener("bounds_changed", () => {
    roteBox_to.setBounds(map.getBounds());
  });
  ...
  //予測変換
  autocomplete = new google.maps.places.Autocomplete(input_a, options);
  autocomplete = new google.maps.places.Autocomplete(input_b, options);
